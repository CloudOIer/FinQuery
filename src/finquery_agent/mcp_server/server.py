...
除工具外还提供两类只读上下文,两者都由人触发而非模型自主调用:
- Resource:公司名单与表结构。用户在提问前把它挂进上下文,可显著减少因公司名、
  指标名臆造而产生的无效调用。注意 MCP 的资源由客户端应用决定何时读取,
  模型没有"读资源"这个动作,因此实体校对在模型侧仍依赖工具回传的错误信息。
- Prompt:把"多家公司横向对比"这类需要固定调用顺序的套路沉淀为模板,
  由用户显式触发,而不是指望模型每次自行推导出正确的编排。
资源与提示词的 name/description 会被客户端直接渲染为菜单文案,面向最终用户书写。
"""
...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "公司名不确定时直接用常见简称或股票代码尝试取数,工具会回传实体是否在库内。"
    ),
)
